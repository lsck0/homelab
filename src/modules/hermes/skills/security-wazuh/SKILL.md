...
## Wazuh (vm-107, dashboard https://wazuh.lsck0.dev)
...
- Stack status: `ssh 10.100.0.107 'docker ps; systemctl status wazuh'`
- Recent alerts: `ssh 10.100.0.107 'docker exec single-node-wazuh.manager-1 tail -n 50 /var/ossec/logs/alerts/alerts.json' | jq -c '{time:.timestamp, level:.rule.level, desc:.rule.description, host:.predecoder.hostname}'`
- High-level only: filter `.rule.level >= 10`.
- Indexer query (last 24 h, level ≥ 10): `ssh 10.100.0.107 "curl -sk -u admin:\$(cat /var/lib/wazuh/admin-pass) https://localhost:9200/wazuh-alerts-*/_search -H 'Content-Type: application/json' -d '{\"size\":20,\"sort\":[{\"timestamp\":\"desc\"}],\"query\":{\"range\":{\"rule.level\":{\"gte\":10}}}}'"`
- Service health: `ssh 10.100.0.107 docker logs --tail 50 single-node-wazuh.manager-1`.
