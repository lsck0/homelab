{ pkgs, nasMount, ... }: {
  ...
  systemd.services.forgejo-runner-register = {
    ...
    script = ''
      ...
      sed -i '/^container:/,/^[^ ]/{s|^  options: .*|  options: "--add-host=git.lsck0.dev:10.100.0.100 --add-host=registry.lsck0.dev:10.100.0.109 --add-host=sccache.lsck0.dev:10.100.0.106"|}' \
