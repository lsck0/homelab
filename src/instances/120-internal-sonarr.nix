{ pkgs, nasMount, nasPath, ... }: {
  ...
  fileSystems = nasMount "/var/lib/homepage-tokens" "homepage-tokens"
    // nasMount "/var/lib/sonarr" "sonarr"
  ...
  systemd.services.sonarr-homepage-token = {
    description = "Export Sonarr API key for Homepage";
    after = [ "podman-sonarr.service" ];
    wantedBy = [ "multi-user.target" ];
    serviceConfig = { Type = "oneshot"; RemainAfterExit = true; };
    script = ''
      TOKEN_FILE="/var/lib/homepage-tokens/sonarr-key.token"
