...
locals {
  instances = {
    ...
    "126" = { name = "126-internal-hermes", type = "internal", memory = 12288, cores = 8, disk = 60, enabled = false,
              machine = "q35", hostpci = ["gpu"] }                                                # GPU LLM agent (Ollama on the passed-through RTX 2060)
    ...
    "132" = { name = "132-internal-smtp", type = "internal", enabled = false }                    # Postfix null-client SMTP relay
